...
@cli.command(
    help="Train a neural network on MNIST using configurable layers, activations, optimizer and loss. Use --save-model to save the trained model weights to disk."
)
@click.option(
    "--layers",
    default="784,128,64,10",
    help="Comma-separated layer sizes, including input and output. Example: 784,256,128,10",
)
@click.option(
    "--activation",
    default="relu,relu,linear",
    help="Comma-separated activation functions, one per layer transition. Supported: relu, sigmoid, tanh, linear",
)
@click.option(
    "--loss",
    default="cross_entropy",
    help="Loss function to train with. Supported: cross_entropy, mse_loss, nll_loss",
)
@click.option(
    "--optimizer", default="adam", help="Optimizer to use. Supported: adam, sgd"
)
@click.option(
    "--lr", default=0.001, type=float, help="Learning rate for training updates"
)
...
@click.option(
    "--save-model",
    default=None,
    help="Optional path to save trained model weights (e.g. model.pt). The model will be saved after training completes.",
)
def train(
    layers: str,
    activation: str,
    ...
    for epoch in range(epochs):
        ...
        with click.progressbar(
            train_loader, label=f"Epoch {epoch + 1}/{epochs}"
        ) as bar:
...
@click.option(
    "--layers",
    default="784,128,64,10",
    help="Comma-separated layer sizes matching the saved model architecture",
)
