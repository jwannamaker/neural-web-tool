...
def train() -> Union[Response, Tuple[Response, int]]:
    ...
    if len(activations) != len(layer_sizes) - 1:
        return jsonify(
            {
                "error": "Number of activation functions must be one less than number of layers."
            }
        ), 400
...
def evaluate() -> Union[Response, Tuple[Response, int]]:
    ...
    return jsonify({"accuracy": accuracy, "class_accuracies": class_accuracies})
...
def predict_image() -> Union[Response, Tuple[Response, int]]:
    ...
    try:
        ...
        for idx in top_indices:
            results.append({"digit": int(idx), "confidence": float(probs_list[idx])})
